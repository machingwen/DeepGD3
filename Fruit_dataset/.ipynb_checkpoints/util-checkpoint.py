...
def CreateDataset_relabel(seed, testing=None):
    ...
    df = pd.read_csv("~/Phison/dataset_relabel.csv")
...
#         test_df_bad = test_df.loc[test_df["class"]==1].groupby('component_name', group_keys=False).apply(lambda x: x.sample(frac=0.01, random_state=123)) 
...
def get_fruit_8(seed):
    ...
    # 隨機挑選2類 bad sample 作為 unseen 的資料
    ...
    per_component_num = 128 // len(train_com_dataset.dataframe['component_name'].value_counts().index)  # 128//總共幾類
    ...
    val_com_loader = torch.utils.data.DataLoader(
        val_com_dataset, batch_size=32, shuffle=False,
        num_workers=8, pin_memory=True)
    ...
    return input_size ,num_classes ,train_com_loader, train_cls_loader, val_loader, val_com_loader, test_loader, train_cls_df, train_com_df, val_df, test_df
