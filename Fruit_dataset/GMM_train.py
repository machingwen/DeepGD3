...
#         raise ValueError(opt.dataset)
    ...
    if opt.dataset == 'fruit_8':
        ...
        train_loader = CreateDataset_for_each_component_regroup(opt.seed, train_com_df, opt.componentName)
...
def main():
    ...
    PATH = f"/root/notebooks/nfs/work/yanwei.liu/GMM/HybridExpert+_fruit_8/batch_size128_gmm{opt.gaussian_num}/dataset_{opt.seed}_GMM_{opt.componentName}_Classifier/"
    if not os.path.exists(PATH):
        os.makedirs(PATH)
    gmm=GaussianMixture(num_components=opt.gaussian_num, covariance_type='full', trainer_params=dict(gpus=1), covariance_regularization=1e-1)
    try:
        gmm.fit(features)
        gmm.save(PATH)
    except:
