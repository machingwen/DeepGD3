...
def plot_scatter_all(args, X, y, X_test, y_test):    
    ...
    ax.axis("off")
    ax.legend(loc='best')
    #tnse_points_path= os.path.join(args["output_dir"], "{}/{}_train+test_component_HBE+_component_tsne.pdf".format(args["random_seed"], args["embedding_layer"]))
    tnse_points_path= "{}_{}_train+test_component_HBE+_component_tsne.pdf".format(args["random_seed"], args["embedding_layer"])
...
def plot_scatter(args, X, y, X_test, y_test):    
    ...
    ax.axis("off")
    ax.legend(loc='best')
    #tnse_points_path= os.path.join(args["output_dir"], "{}/{}_train+test_component_HBE+_class_tsne.pdf".format(args["random_seed"], args["embedding_layer"]))
    tnse_points_path= "{}_{}_train+test_component_HBE+_class_tsne.pdf".format(args["random_seed"], args["embedding_layer"])
    plt.savefig(tnse_points_path, dpi=150)
...
if __name__ == "__main__":
    ...
    training_loader, _, test_loader = CreateTSNEdatasetRelabel(args["random_seed"], gmm=True)
