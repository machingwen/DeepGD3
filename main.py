...
from util import set_optimizer_two, save_model, CreateDataset_relabel, CustomDataset
...
def set_loader(opt):
    ...
    if opt.dataset == 'phison':
        train_cls_df, val_df, test_df, _, _, _, train_com_df = CreateDataset_relabel(opt.seed)
        
        train_cls_dataset = CustomDataset(train_cls_df, 
                                      transform=train_transform)
        
        train_com_dataset = CustomDataset(train_com_df, 
                                      transform=train_com_transform)        
    ...
    return train_com_loader, train_cls_loader, val_loader, test_loader
...
def validate(val_loader, model, criterion_com, criterion_cls, opt):
    """validation"""
    model.eval()
        ...
        for idx, (images, labels, _, component_name) in enumerate(val_loader):
            ...
            if idx % opt.print_freq == 0:
                print('Test: [{0}/{1}]\t'
                      ...
                      'Loss component {loss_component.val:.4f} ({loss_component.avg:.4f})\t'
                      'Class Acc@1 {top1.val:.3f} ({top1.avg:.3f})\t'.format(
                       idx, len(val_loader), batch_time=batch_time,
                       loss=losses, loss_component=losses_component, top1=top1))
...
def main():
    ...
    train_com_loader, train_cls_loader, val_loader, test_loader = set_loader(opt)
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        # eval for one epoch
        val_cls_loss, val_component_loss, val_acc = validate(val_loader, model, criterion_com, criterion_cls, opt)
